...
| `APP_ENV` | rendered | the `observability.deployment.environment` discovery label; the canonical environment name, never the branch |
...
| `APP_ENV` | compose-level only — the `observability.deployment.environment` discovery label |
